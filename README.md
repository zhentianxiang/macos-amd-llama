...
进入页面后，在模型列表中选择要加载的模型，控制代理会自动启动对应的
llama-server（默认 `127.0.0.1:8080`）。按 `Ctrl+C` 会同时停止控制代理和
Web 服务，面板加载的模型也会随之停止。8080 端口已有手工启动的 llama-server
时，面板会直接使用它，而不会重复启动。
...
`127.0.0.1:8090`。模型由面板加载后，llama-server 监听 `127.0.0.1:8080`；
图片向量服务需要单独运行 `make embedding`，监听 `127.0.0.1:8092`。如果 8080
端口已有手工启动的 llama-server，面板会显示它处于在线状态，但不会停止或替换
该外部进程，以避免误杀其他服务。
...
`CTX_SIZE` 默认为 `16384` tokens，是历史消息、当前问题和模型回答共享的总窗口；
`CHAT_MAX_TOKENS` 默认为 `1024`。默认关闭隐藏思考以缩短首字等待时间；复杂
推理任务可以设置 `CHAT_ENABLE_THINKING=true`，思考过程同样会占用单次输出
token，页面会显示“正在思考”，达到输出上限时会明确提示继续生成。
如果出现内存不足，可把上下文降为 `8192` 或 `4096`。

聊天窗口提供“思考”开关。开启后，支持 reasoning 的模型会把思考内容实时
显示在可折叠的“思考过程”面板中；关闭时优先缩短首字等待时间。

当前模型带有 `VISION` 标记时，聊天输入框会启用“上传图片”按钮。支持
JPEG、PNG、GIF、WebP 和 BMP，单张图片最大 6 MB。选择图片后可以先查看
预览，再输入问题；如果不输入文字，默认让模型描述图片。图片会保留在当前
对话上下文中，使用“清空上下文”可以移除历史图片和问答。

从 Web 控制台启动 Gemma 4 26B A4B 或 Gemma 4 31B 时，代理会检测系统内存，
并自动使用 `GPU_LAYERS=auto`、Q8 KV Cache 和 CPU/GPU 内存分担。32 GB
系统内存下，26B 默认使用 8192 上下文并预留 1536 MiB 显存；31B 默认使用
4096 上下文并预留 2048 MiB 显存。视觉投影放在系统内存中，不需要手工修改
`.env`。聊天窗口顶部会显示当前采用的运行配置。
